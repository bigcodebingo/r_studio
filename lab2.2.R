...
par(mar = c(12,4,4,2))
boxplot(data, main = "boxplot для всех ресторанов", xlab = "Рестораны", ylab = "Значения", col = "lightblue",las = 2)
par(mar = c(5,4,4,2))

data_stats <- summary(data)
print(data_stats)
#----------------------------------------------
data_sort <- data[order(data$Уни), ]
#----------------------------------------------
tori_selected <- subset(data, Tori.ramen > 7)
print(tori_selected)
...
ggplot(tori_selected, aes(x = as.factor(Tori.ramen))) +
  geom_bar(fill = "black", color = "black", position = "dodge") +
  labs(title = "Рис.1. Гистограмма всех оценок Tori.ramen", x = "Оценка Tori.ramen", y = "Частота")
#-----------------------------------------------
